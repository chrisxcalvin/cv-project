...
def init_db():
    ...
    """)
    # Migrate older DBs (created before ear_min/yaw_max_delta/spoof_type existed).
    existing_cols = {row["name"] for row in conn.execute("PRAGMA table_info(attempts)")}
    for col in ("ear_min", "yaw_max_delta"):
...
def log_attempt(passive_result: dict, decision_result: dict, active_result: dict = None):
    ...
    passive_result:  dict returned by PassiveClassifier.predict()
